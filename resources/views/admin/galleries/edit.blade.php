<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            Edit Gallery: <span class="fw-light">{{ $gallery->title }}</span>
        </h2>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    ...
                    {{-- This form submits to the 'update' method --}}
                    <form action="{{ route('admin.galleries.update', $gallery) }}" method="POST">
                        @csrf
                        @method('PUT') {{-- This tells Laravel it's an UPDATE, not a create --}}

                        <div class="mb-3">
                            <label for="title" class="form-label">Title</label>
                            {{-- We pre-fill the 'value' with the gallery's current title --}}
                        ...
                        <div class="form-check mb-3">
                            <input class="form-check-input" type="checkbox" id="is_visible" name="is_visible" value="1"
                                {{-- We check the box if the gallery is currently visible --}}
