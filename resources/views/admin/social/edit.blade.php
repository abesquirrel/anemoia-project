...
@section('content')
    <div class="container-fluid px-4">
        <h1 class="mt-4">Edit Social Platform</h1>
        <ol class="breadcrumb mb-4">
            <li class="breadcrumb-item"><a href="{{ route('admin.dashboard') }}">Dashboard</a></li>
            <li class="breadcrumb-item"><a href="{{ route('admin.social-platforms.index') }}">Social Settings</a></li>
            <li class="breadcrumb-item active">Edit {{ $socialPlatform->name }}</li>
        </ol>

        <div class="card mb-4">
            <div class="card-body">
                <form action="{{ route('admin.social-platforms.update', $socialPlatform) }}" method="POST">
                    @csrf
                    @method('PUT')
                    <div class="mb-3">
                        <label for="name" class="form-label">Name</label>
                        <input type="text" class="form-control @error('name') is-invalid @enderror" id="name" name="name"
                            value="{{ old('name', $socialPlatform->name) }}" required>
                        @error('name')
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control @error('slug') is-invalid @enderror" id="slug" name="slug"
                            value="{{ old('slug', $socialPlatform->slug) }}" required>
                        @error('slug')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="url" class="form-control @error('base_share_url') is-invalid @enderror"
                            id="base_share_url" name="base_share_url"
                            value="{{ old('base_share_url', $socialPlatform->base_share_url) }}" required>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control @error('icon_class') is-invalid @enderror" id="icon_class"
                            name="icon_class" value="{{ old('icon_class', $socialPlatform->icon_class) }}" required>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="color" class="form-control form-control-color @error('color') is-invalid @enderror"
                            id="color" name="color" value="{{ old('color', $socialPlatform->color) }}">
                        @error('color')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="mb-3 form-check">
                        <input type="checkbox" class="form-check-input" id="is_active" name="is_active" value="1" {{ old('is_active', $socialPlatform->is_active) ? 'checked' : '' }}>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="number" class="form-control @error('sort_order') is-invalid @enderror" id="sort_order"
                            name="sort_order" value="{{ old('sort_order', $socialPlatform->sort_order) }}">
                        @error('sort_order')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
